...
│   پلتفرم: Termux / Linux                         │
│   سازنده: @Reyvoxe                               │
│   Local Control Tower: http://localhost:8880      │
╰──────────────────────────────────────────────────╯
...
#  واترمارک و ثوابت
# ══════════════════════════════════════════════
WATERMARK = "💎 ONYX SELF V.11 | سازنده: @Reyvoxe"
VERSION   = "11.0.0"
PROJECT   = "ONYX SELF V.11"
...
DB_PATH  = os.path.join(BASE_DIR, "onyx_v11.db")
...
# ── Clock State Machine V11 ───────────────────────────
# این متغیرها در بخش LCT (پایین فایل) مقداردهی می‌شوند
# اینجا فقط forward-declare می‌کنیم
_clock_state: str            = "DISABLED"
_clock_last_tick: float      = 0.0
_clock_fail_reason: str      = ""
_clock_consecutive_errors: int = 0

async def _clock_loop_v10(client) -> None:
    """حلقه ساعت V11 — با State Machine کامل و Health Monitoring"""
    global _clock_task, _clock_state, _clock_last_tick, _clock_fail_reason, _clock_consecutive_errors
    _clock_state = "STARTING"
    _clock_set_active(True)
    _clock_consecutive_errors = 0
    logger.info("⏰ V11: Clock loop started")
    await asyncio.sleep(1)
    _clock_state = "RUNNING"
    while True:
        try:
            now = iran_now()
            time_str = now.strftime("%H:%M")
            ...
            _clock_last_tick = _time.time()
            _clock_consecutive_errors = 0
            _clock_state = "RUNNING"
            _clock_fail_reason = ""
            # log to activity feed (function defined later in LCT section)
            try:
                _lct_activity("CLOCK", f"Tick ✓ {time_str} | {date_str}")
            except NameError:
                pass
        except FloodWaitError as e:
            ...
            _clock_state = "STOPPING"
            break
        except Exception as e:
            _clock_consecutive_errors += 1
            _clock_fail_reason = str(e)
            logger.warning(f"⏰ Clock error #{_clock_consecutive_errors}: {e}")
            try:
                _bb_log_v9("clock_error", str(e)[:100], "error")
            except (NameError, Exception):
                pass
            if _clock_consecutive_errors >= 5:
                _clock_state = "FAILED"
                logger.error("⏰ Clock stopped after 5 errors")
                break
            _clock_state = "RUNNING"
            await asyncio.sleep(min(30 * _clock_consecutive_errors, 120))
            continue
        await asyncio.sleep(60)
    _clock_state = "DISABLED"
    _clock_set_active(False)
    logger.info("⏰ V11: Clock loop stopped")

async def _clock_health_check_loop(client) -> None:
    """بررسی سلامت ساعت هر ۵ دقیقه"""
    global _clock_task, _clock_state
    ...
    while True:
        try:
            ...
            now  = _time.time()
            # بررسی FAILED
            if _clock_state == "FAILED" and _clock_is_active():
                logger.warning("🛡️ Clock Health: FAILED — auto-recovering...")
                if task and not task.done():
                    task.cancel()
                    await asyncio.sleep(2)
                _clock_task = asyncio.create_task(_clock_loop_v10(client))
                try:
                    _bb_log_v9("clock_recovered","Health Check auto-recovery","info")
                    _lct_activity("CLOCK","Health Check: auto-recovered")
                except NameError:
                    pass
            # بررسی task مرده ولی باید روشن باشد
            elif (task is None or task.done()) and _clock_is_active() and _clock_state not in ("STARTING","RUNNING"):
                logger.info("🛡️ Clock Health: task dead, restoring...")
                _clock_task = asyncio.create_task(_clock_loop_v10(client))
                try:
                    _bb_log_v9("clock_restored","Clock auto-restored by health","info")
                    _lct_activity("CLOCK","Health Check: task restored")
                except NameError:
                    pass
        except Exception as e:
            logger.debug(f"ClockHealth: {e}")
...
def register_all(client):
    ...
    async def clock_on(event):
        ...
        if _clock_task and not _clock_task.done():
            _clock_task.cancel()
            try:
                await asyncio.wait_for(asyncio.shield(_clock_task), timeout=3)
            except Exception:
                pass
        _clock_task = None
        await asyncio.sleep(0.5)
        ...
        fn  = setting("clock_font", "normal")
        now = iran_now()
        real_state = _lct_get_clock_real_status()
        st  = real_state["state"]
        ok  = st in ("RUNNING","STARTING")
        await safe_edit(event, box(
            "⏰ ساعت " + ("فعال شد ✅" if ok else "شروع با مشکل ⚠️"),
            [
                f"وضعیت واقعی: {st}",
                ...
                "خاموش: ساعت_خاموش",
                "وضعیت: وضعیت_ساعت",
                "پنل: http://localhost:8880/clock",
            ] + ([f"⚠️ خطا: {real_state.get('fail_reason','')[:60]}"] if not ok else [])
        ))
    ...
    async def clock_off(event):
        ...
        _clock_state = "STOPPING"
        _clock_set_active(False)
        was_running = bool(_clock_task and not _clock_task.done())
        if _clock_task and not _clock_task.done():
            _clock_task.cancel()
            try:
                await asyncio.wait_for(asyncio.shield(_clock_task), timeout=3)
        ...
        _clock_task  = None
        _clock_state = "DISABLED"
        msg = "✅ ساعت خاموش شد." if was_running else "ℹ️ ساعت از قبل خاموش بود."
    ...
    async def clock_status(event):
        ...
        real = _lct_get_clock_real_status()
        state   = real["state"]
        fn      = real["font"]
        now     = iran_now()
        date_fa = jalali(now)
        time_now = now.strftime("%H:%M:%S")
        lt_ago  = real.get("last_tick_ago", -1)
        tick_str = f"{lt_ago}s پیش" if lt_ago >= 0 else "هرگز"
        try:
            preview = CLOCK_FONTS.get(fn, CLOCK_FONTS["normal"])(now.strftime("%H:%M"))
        except Exception:
            preview = now.strftime("%H:%M")
        icon = {"RUNNING":"🟢","STARTING":"🟡","FAILED":"🔴","STOPPING":"🟠","DISABLED":"⚫"}.get(state,"⚫")
        lines = [
            f"وضعیت واقعی: {icon} {state}",
            f"آخرین Tick: {tick_str}",
            f"فونت: {fn}",
            f"تاریخ: {date_fa}",
            f"ساعت: {time_now}",
            f"پیش‌نمایش: {preview}",
            f"ذخیره در DB: {'بله' if real.get('saved_active') else 'خیر'}",
        ]
        if real.get("fail_reason"):
            lines.append(f"⚠️ خطا: {real['fail_reason'][:60]}")
        lines.append("پنل: http://localhost:8880/clock")
        await safe_edit(event, box("⏰ وضعیت ساعت V10", lines))
...
# ══════════════════════════════════════════════════════
#  ═══  ONYX SELF V10 — ارتقاء کامل  ═══
#  تمام قابلیت‌های جدید — تک‌فایلی — بدون تغییر ساختار
# ══════════════════════════════════════════════════════
# VERSION already set to 11.0.0 at top
...
# ══════════════════════════════════════════════════════
#  ═══  AUTO SETUP SYSTEM — ONYX V.11  ═══
# ══════════════════════════════════════════════════════

CONFIG_FILE  = BASE_DIR / "config.ini"
SESSION_NAME = "onyx_v11"
...
def _mask(s: str, show: int = 4) -> str:
    """نمایش Masked برای اطلاعات حساس"""
    if not s:
        return "─ تنظیم نشده ─"
    if len(s) <= show * 2:
        return "•" * len(s)
...
def _check_and_install_deps() -> bool:
    ...
    print("\n┌─────────────────────────────────────────────────┐")
    print("│  📦  نصب خودکار کتابخانه‌های مورد نیاز          │")
    print("└─────────────────────────────────────────────────┘")
    for import_name, pkg_name in missing:
        print(f"  ⏳ نصب: {pkg_name} ...")
        ret = os.system(f"{sys.executable} -m pip install {pkg_name} -q")
        if ret != 0:
            print(f"  ❌ نصب {pkg_name} ناموفق بود.")
            print(f"     دستی اجرا کنید: pip install {pkg_name}")
            return False
        try:
            ...
            print(f"  ❌ {pkg_name} نصب شد اما قابل import نیست.")
            print("     لطفاً دستی نصب کنید یا محیط Python را بررسی کنید.")
            return False
    return True

def _load_config() -> dict:
    """بارگذاری تنظیمات از config.ini یا متغیرهای محیطی"""
    cfg = {
        "api_id":      os.environ.get("ONYX_API_ID",   ""),
        "api_hash":    os.environ.get("ONYX_API_HASH",  ""),
        "session":     os.environ.get("ONYX_SESSION",   SESSION_NAME),
        "phone":       os.environ.get("ONYX_PHONE",     ""),
        "gemini_key":  os.environ.get("ONYX_GEMINI",    ""),
        "timezone":    os.environ.get("ONYX_TZ",        "Asia/Tehran"),
    }
    if CONFIG_FILE.exists():
        ...
        cfg["gemini_key"] = sec.get("gemini_key",  cfg["gemini_key"])
        cfg["timezone"]   = sec.get("timezone",    cfg["timezone"])
    return cfg

def _save_config(cfg: dict) -> None:
    """ذخیره تنظیمات در config.ini — بدون هیچ Secret در سورس"""
    # فقط فیلدهای مجاز ذخیره می‌شوند؛ Session جداگانه در فایل .session است
    parser = configparser.ConfigParser()
    parser["onyx"] = {
        "api_id":    cfg.get("api_id",    ""),
        "api_hash":  cfg.get("api_hash",  ""),
        "session":   cfg.get("session",   SESSION_NAME),
        "phone":     cfg.get("phone",     ""),
        "gemini_key": cfg.get("gemini_key", ""),
        "timezone":  cfg.get("timezone",  "Asia/Tehran"),
    }
    with open(str(CONFIG_FILE), "w", encoding="utf-8") as f:
        parser.write(f)

def _is_configured() -> bool:
    """تشخیص اینکه آیا سلف قبلاً تنظیم شده یا نه"""
    cfg = _load_config()
    return bool(cfg.get("api_id") and cfg.get("api_hash"))

def _test_gemini(api_key: str) -> tuple:
    """تست اتصال Gemini — بدون چاپ کردن کلید"""
    if not api_key:
        return False, "کلید وارد نشده"
    try:
        import urllib.request as _ureq
        url = f"https://generativelanguage.googleapis.com/v1beta/models?key={api_key}"
        req = _ureq.urlopen(url, timeout=8)
        data = json.loads(req.read().decode())
        if "models" in data:
            return True, "اتصال موفق"
        return False, "پاسخ غیرمنتظره"
    except Exception as e:
        msg = str(e)
        # هرگز کلید را در پیام خطا نشان نده
        if api_key in msg:
            msg = msg.replace(api_key, "***")
        return False, msg[:80]

def _run_setup_wizard(rerun: bool = False) -> dict:
    """
    Setup Wizard — دریافت اطلاعات از کاربر به شکل مرتب.
    اطلاعات هرگز در سورس یا Log قرار نمی‌گیرند.
    """
    import getpass

    print()
    print("╔══════════════════════════════════════════════════╗")
    print("║      💎 ONYX SELF V.11 — Setup Wizard           ║")
    if rerun:
        print("║      (اجرای مجدد Setup)                         ║")
    print("╚══════════════════════════════════════════════════╝")
    print()
    print("  برای دریافت API ID و HASH:")
    print("  ← https://my.telegram.org → API development tools")
    print()

    # ── بارگذاری مقادیر فعلی (برای rerun) ──
    old_cfg = _load_config() if rerun else {}

    def _ask(prompt: str, secret: bool = False, default: str = "", old_key: str = "") -> str:
        """دریافت ورودی با نمایش مقدار قدیمی در صورت rerun"""
        hint = ""
        if rerun and old_key and old_cfg.get(old_key):
            old_val = old_cfg[old_key]
            hint = f" [فعلی: {_mask(old_val)}، Enter برای حفظ]"
        full_prompt = f"  {prompt}{hint}: "
        if secret:
            val = getpass.getpass(full_prompt)
        else:
            val = input(full_prompt).strip()
        if not val and rerun and old_key:
            val = old_cfg.get(old_key, "")
        return val

    # ── مرحله ۱: API ID ──
    print("─── مرحله ۱: Telegram API ───────────────────────────")
    while True:
        api_id = _ask("API ID", old_key="api_id")
        if api_id.isdigit():
            break
        print("  ❌ API ID باید فقط عدد باشد.")

    # ── مرحله ۲: API HASH ──
    while True:
        api_hash = _ask("API HASH", secret=True, old_key="api_hash")
        if len(api_hash) >= 10:
            break
        print("  ❌ API HASH نامعتبر است.")

    # ── مرحله ۳: شماره تلفن ──
    print()
    print("─── مرحله ۳: ورود به تلگرام ─────────────────────────")
    while True:
        phone = _ask("شماره تلفن (مثال: +989123456789)", old_key="phone")
        if phone.startswith("+") and len(phone) > 7:
            break
        print("  ❌ شماره باید با + شروع شود.")

    # ── مرحله ۴: Gemini API Key ──
    print()
    print("─── مرحله ۴: Gemini AI (اختیاری) ───────────────────")
    print("  اگر Gemini ندارید، Enter بزنید (سلف بدون آن هم کار می‌کند).")
    gemini_key = _ask("Gemini API Key", secret=True, old_key="gemini_key")

    gemini_ok = False
    if gemini_key:
        print("  ⏳ تست اتصال Gemini...")
        gemini_ok, gemini_msg = _test_gemini(gemini_key)
        if gemini_ok:
            print(f"  ✅ Gemini: {gemini_msg}")
        else:
            print(f"  ⚠️  Gemini: {gemini_msg}")
            cont = input("  ادامه بدهم؟ [y/N]: ").strip().lower()
            if cont != "y":
                gemini_key = ""

    # ── مرحله ۵: Timezone ──
    print()
    print("─── مرحله ۵: Timezone ───────────────────────────────")
    old_tz = old_cfg.get("timezone", "Asia/Tehran") if rerun else "Asia/Tehran"
    tz_input = input(f"  Timezone [{old_tz}]: ").strip()
    timezone = tz_input if tz_input else old_tz
    ...
    print("  ⏳ ذخیره تنظیمات...")
    _save_config(cfg)
    print(f"  ✅ تنظیمات ذخیره شد: {CONFIG_FILE}")
    print()

    return cfg

def _print_startup_status(cfg: dict) -> None:
    """نمایش وضعیت Startup به شکل مرتب"""
    session_file = BASE_DIR / f"{cfg.get('session', SESSION_NAME)}.session"
    ...
    print()
    print("╔══════════════════════════════════════════════════╗")
    print("║           💎 ONYX SELF V.11 STARTING            ║")
    print("╠══════════════════════════════════════════════════╣")
    print(f"  [✓] Configuration loaded")
    print(f"  [{'✓' if session_ok else '─'}] Telegram Session {'loaded' if session_ok else 'will be created'}")
    print(f"  [✓] Dependencies verified")
    print(f"  [{'✓' if gemini_set else '─'}] AI / Gemini {'configured' if gemini_set else 'not configured'}")
    print(f"  [✓] Timezone: {cfg.get('timezone','Asia/Tehran')}")
    print("╠══════════════════════════════════════════════════╣")
    print("║           💎 ONYX SELF ONLINE                   ║")
    print("╚══════════════════════════════════════════════════╝")
    print()

def _handle_setup_flag() -> None:
    """اجرای --setup برای تغییر تنظیمات"""
    print()
    print("╔══════════════════════════════════════════════════╗")
    print("║        💎 ONYX SELF V.11 — مدیریت تنظیمات      ║")
    print("╚══════════════════════════════════════════════════╝")

    cfg = _load_config()
    if not cfg.get("api_id"):
        print("  ℹ️  هنوز Setup نشده. اجرای Setup Wizard...")
        _run_setup_wizard(rerun=False)
        return

    print()
    print("  تنظیمات فعلی:")
    print(f"    API ID    : {_mask(cfg.get('api_id',''))}")
    print(f"    API HASH  : {_mask(cfg.get('api_hash',''))}")
    print(f"    Phone     : {_mask(cfg.get('phone',''))}")
    print(f"    Gemini    : {_mask(cfg.get('gemini_key',''))}")
    print(f"    Timezone  : {cfg.get('timezone','Asia/Tehran')}")
    session_file = BASE_DIR / f"{cfg.get('session', SESSION_NAME)}.session"
    print(f"    Session   : {'✅ موجود' if session_file.exists() else '─ موجود نیست'}")
    print()
    print("  چه کاری می‌خواهید انجام دهید؟")
    print("   [1] تغییر همه تنظیمات (Setup Wizard)")
    print("   [2] فقط تغییر Gemini API Key")
    print("   [3] فقط تغییر Timezone")
    print("   [4] Reset Session (نیاز به Login مجدد)")
    print("   [5] خروج بدون تغییر")
    print()

    choice = input("  انتخاب [1-5]: ").strip()

    if choice == "1":
        _run_setup_wizard(rerun=True)

    elif choice == "2":
        import getpass
        print()
        new_key = getpass.getpass("  Gemini API Key جدید (Enter برای حذف): ")
        if new_key:
            ok, msg = _test_gemini(new_key)
            if ok:
                print(f"  ✅ اتصال موفق: {msg}")
            else:
                print(f"  ⚠️  {msg}")
                cont = input("  ذخیره کنم؟ [y/N]: ").strip().lower()
                if cont != "y":
                    new_key = cfg.get("gemini_key", "")
        cfg["gemini_key"] = new_key
        _save_config(cfg)
        print("  ✅ ذخیره شد.")

    elif choice == "3":
        old_tz = cfg.get("timezone", "Asia/Tehran")
        new_tz = input(f"  Timezone جدید [{old_tz}]: ").strip()
        cfg["timezone"] = new_tz if new_tz else old_tz
        _save_config(cfg)
        print("  ✅ Timezone ذخیره شد.")

    elif choice == "4":
        confirm = input("  ⚠️  Session حذف خواهد شد و نیاز به Login مجدد دارید. مطمئنید؟ [y/N]: ").strip().lower()
        if confirm == "y":
            for ext in [".session", ".session-journal"]:
                sess_path = BASE_DIR / f"{cfg.get('session', SESSION_NAME)}{ext}"
                if sess_path.exists():
                    sess_path.unlink()
                    print(f"  🗑️  {sess_path.name} حذف شد.")
            print("  ✅ Session Reset شد. دفعه بعد Login دوباره انجام می‌شود.")
        else:
            print("  ↩️  لغو شد.")

    elif choice == "5":
        print("  ↩️  بدون تغییر خارج شد.")
    else:
...
def load_config() -> dict:
    """
    بارگذاری تنظیمات — ابتدا config.ini، سپس env vars.
    این تابع برای سازگاری با کد اصلی حفظ شده.
    """
    return _load_config()

def prompt_config() -> dict:
    """
    راه‌اندازی اولیه — این تابع برای سازگاری با کد اصلی حفظ شده.
    در نسخه V.11 از Setup Wizard استفاده می‌شود.
    """
    return _run_setup_wizard(rerun=False)
...
# ── راه‌اندازی اصلی ───────────────────────────
async def main():
    global _lct_client_ref, _lct_me_ref
    ...
    if not cfg.get("api_id") or not cfg.get("api_hash"):
        ...
        cfg = _run_setup_wizard(rerun=False)

    if not cfg.get("api_id"):
        print("❌ API ID الزامی است — خروج."); sys.exit(1)

    # ── نمایش وضعیت Startup ──────────────────
    _print_startup_status(cfg)

    print(f"  💎 ONYX SELF {PROJECT}")
    ...
    # ── V10: ذخیره reference برای LCT ──────
    _lct_client_ref = client
    _lct_me_ref     = me

    # ── V10: شروع Local Control Tower Server ──
    lct_thread = threading.Thread(target=_start_lct_server, daemon=True, name="LCT-WebServer")
    lct_thread.start()
    ...
    # ── V10: ساعت — بازیابی خودکار + Health Check ──
    try:
        if _clock_is_active():
            logger.info("⏰ V10: بازیابی خودکار ساعت از دیتابیس...")
            _clock_task = asyncio.create_task(_clock_loop_v10(client))
            background_tasks.append(("clock_v10", _clock_task))
    except Exception as e:
        logger.warning(f"⏰ خطا در بازیابی ساعت V10: {e}")

    # ── V10: Clock Health Check loop ────────
    t = asyncio.create_task(_clock_health_check_loop(client))
    background_tasks.append(("clock_health_v10", t))
    ...
    clock_status_v10 = _lct_get_clock_real_status()
    clock_note = ""
    if _clock_is_active():
        clock_note = f"⏰ ساعت بازیابی شد — وضعیت: {clock_status_v10['state']}"
    await client.send_message(me.id, (
        f"💎 **{PROJECT}** آماده است!\n\n"
        ...
        f"🗄️ دیتابیس: {DB_PATH}\n"
        + (f"{clock_note}\n" if clock_note else "")
        + f"\n🖥️ **Local Control Tower V.11:**\n"
        f"http://localhost:{LCT_PORT}\n\n"
        f"✨ V.11 — Professional Control Center:\n"
        f"• 9 صفحه: Dashboard · Features · Commands · Clock · AI · Health · Logs · Activity · Settings\n"
        f"• Real-time Ping نمایش لحظه‌ای\n"
        f"• Global Search سراسری\n"
        f"• AI Provider Manager (Gemini/OpenAI/Claude)\n"
        f"• Clock State Machine (DISABLED/STARTING/RUNNING/FAILED)\n"
        f"• Live Activity Feed\n"
        f"• Dark Professional UI\n\n"
        f"راهنما: **منو** | پنل: http://localhost:{LCT_PORT}\n"
        f"{WATERMARK}"
    ))

    print(f"\n🚀 {PROJECT} آماده است!")
    print(f"📚 {total_cmds} دستور فعال")
    print(f"🖥️  پنل مدیریت: http://localhost:{LCT_PORT}")
...
# ══════════════════════════════════════════════════════════════════════
#  💎 ONYX SELF V.10 — LOCAL CONTROL TOWER
#  Web Panel: http://localhost:8880
#  تک‌فایلی — بدون نیاز به فایل خارجی — بدون تغییر ساختار قبلی
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  💎 ONYX SELF V.11 — PROFESSIONAL LOCAL CONTROL TOWER
#  http://localhost:8880
# ══════════════════════════════════════════════════════════════════════
...
# _clock_state و سایر متغیرهای ساعت در بالای فایل (بعد از _clock_loop) تعریف شده‌اند
_lct_client_ref              = None
_lct_me_ref                  = None
_lct_start_time: float       = _time.time()

# ── Activity Log (in-memory, 200 entries) ────────────
from collections import deque
_activity_log: deque = deque(maxlen=200)

def _lct_activity(category: str, message: str) -> None:
...
def _lct_get_ai_providers() -> list:
    with _db_lock:
        conn = get_conn()
        try:
            rows = conn.execute("SELECT * FROM ai_providers ORDER BY is_default DESC, id").fetchall()
            return [dict(r) for r in rows]
        except Exception:
            return []

def _lct_mask_key(key: str) -> str:
    if not key:
        return "—"
    return key[:4] + "••••••••••••" + key[-3:] if len(key) > 8 else "••••••••"

def _lct_get_clock_real_status() -> dict:
    global _clock_state, _clock_last_tick, _clock_fail_reason
    task = _clock_task
    now  = _time.time()
    if task is None or task.cancelled():
        ...
        try:
            exc = task.exception()
            _clock_state = "FAILED"
            _clock_fail_reason = str(exc) if exc else "Unknown"
        except Exception:
            _clock_state = "DISABLED"
    elif _clock_state == "RUNNING" and _clock_last_tick > 0:
        elapsed = now - _clock_last_tick
        if elapsed > 180:
            _clock_state = "FAILED"
            _clock_fail_reason = f"No tick for {int(elapsed)}s"
    return {
        "state": _clock_state,
        "is_running": _clock_state == "RUNNING",
        "last_tick": _clock_last_tick,
        "last_tick_ago": int(now - _clock_last_tick) if _clock_last_tick > 0 else -1,
        "fail_reason": _clock_fail_reason,
        "font": setting("clock_font", "normal"),
        "saved_active": _clock_is_active(),
    }

def _lct_get_sys_metrics() -> dict:
    m = {}
    try:
        if _HAS_PSUTIL:
            m["cpu"]    = _psutil.cpu_percent(interval=0.1)
            ...
            m["cpu"] = m["ram"] = 0
            m["ram_mb"] = 0
        uptime_secs = int(_time.time() - _lct_start_time)
        h, rem      = divmod(uptime_secs, 3600)
        mins, s     = divmod(rem, 60)
        m["uptime"] = f"{h:02d}:{mins:02d}:{s:02d}"
    except Exception:
        m["cpu"] = m["ram"] = 0
        m["ram_mb"] = 0
        m["uptime"] = "—"
...
# ── FEATURE REGISTRY — pulls directly from FULL_MENU ──
FEATURE_REGISTRY: list = []

def _lct_build_feature_registry() -> list:
    """Build feature registry from FULL_MENU dynamically"""
    CATEGORY_META = {
        "🏠 اصلی / سیستم": ("system",  "System",     "⚙️"),
        "👤 پروفایل":       ("profile", "Profile",    "👤"),
        "👥 مخاطبان":       ("contacts","Contacts",   "👥"),
        "⏰ ساعت و فونت":  ("clock",   "Clock",      "⏰"),
        "📊 تحلیل و آمار":  ("analytics","Analytics", "📊"),
        "🤖 اتوماسیون":    ("automation","Automation","🤖"),
        "🧠 ویژگی‌های هوشمند":("ai",  "AI / Smart", "🧠"),
        "📥 رسانه و دانلود":("media",  "Media",      "📥"),
        "📁 مدیریت فایل":  ("files",   "Files",      "📁"),
        "🔐 امنیت":        ("security","Security",   "🔐"),
        "👁 پایش و مانیتورینگ":("monitor","Monitor", "👁"),
        "🔧 ابزارها":      ("tools",   "Tools",      "🔧"),
        "📋 مدیریت کارها": ("tasks",   "Tasks",      "📋"),
        "📅 تقویم":        ("calendar","Calendar",   "📅"),
        "💰 مدیریت هزینه": ("finance", "Finance",    "💰"),
        "💾 بکاپ و ریستور":("backup",  "Backup",     "💾"),
        "🧩 پلاگین":       ("plugin",  "Plugins",    "🧩"),
        "✨ انیمیشن":      ("anim",    "Animations", "✨"),
        "🎮 بازی‌ها":      ("games",   "Games",      "🎮"),
    }
    registry = []
    seen_ids = set()
    for cat_fa, cmds in FULL_MENU.items():
        meta = CATEGORY_META.get(cat_fa, ("other", cat_fa, "📌"))
        cat_id, cat_en, cat_icon = meta
        feature_id = f"cat_{cat_id}"
        if feature_id in seen_ids:
            feature_id = f"cat_{cat_id}_{len(seen_ids)}"
        ...
        registry.append({
            ...
            "category": cat_id,
            "cmd_count": len(cmds),
            "commands": [(c[0], c[1]) for c in cmds[:8]],
        })
    # اضافه کردن V9/V10 features که در FULL_MENU هستند
    return registry

def _lct_is_feature_disabled(fid: str) -> bool:
    return setting(f"feat_disabled_{fid}", "0") == "1"

# ══════════════════════════════════════════════════════
#  HTML COMPONENTS — Professional Design System
# ══════════════════════════════════════════════════════
...
.stat-card.green::before { background: linear-gradient(90deg, var(--green), #06b6d4); }
...
.stat-card .label { font-size: .72rem; color: var(--text3); text-transform: uppercase; letter-spacing: .05em; margin-bottom: 8px; }
.stat-card .value { font-size: 1.6rem; font-weight: 800; line-height: 1; }
.stat-card .sub   { font-size: .72rem; color: var(--text2); margin-top: 6px; }
.stat-card .icon  { position: absolute; top: 18px; right: 16px; font-size: 1.4rem; opacity: .3; }
...
.fc-name { font-weight: 700; font-size: .92rem; display: flex; align-items: center; gap: 6px; }
.fc-desc { font-size: .76rem; color: var(--text2); line-height: 1.5; margin-bottom: 10px; }
.fc-meta { font-size: .7rem; color: var(--text3); margin-bottom: 10px; }
...
.status-label { flex: 1; color: var(--text2); }
.status-val   { font-weight: 600; display: flex; align-items: center; gap: 5px; }
...
.badge.yellow { background: rgba(245,158,11,.15);  color: var(--yellow2); }
...
.btn-success { background: rgba(16,185,129,.15); color: var(--green2); border-color: rgba(16,185,129,.3); }
...
.table td { padding: 10px 14px; font-size: .81rem; border-bottom: 1px solid var(--border); }
...
.alert.ok     { background: rgba(16,185,129,.08);  border-color: var(--green);  color: var(--green2); }
.alert.warn   { background: rgba(245,158,11,.08);  border-color: var(--yellow); color: var(--yellow2); }
.alert.danger { background: rgba(239,68,68,.08);   border-color: var(--red);    color: var(--red2); }
.alert.info   { background: rgba(99,102,241,.08);  border-color: var(--accent); color: var(--accent3); }
...
.log-line .log-ts   { color: var(--text3); white-space: nowrap; }
.log-line .log-cat  { width: 70px; flex-shrink: 0; font-weight: 700; }
.log-line .log-msg  { color: var(--text); flex: 1; }
.log-line.error  { background: rgba(239,68,68,.08); }
.log-line.error .log-cat   { color: var(--red2); }
.log-line.warning { background: rgba(245,158,11,.08); }
.log-line.warning .log-cat { color: var(--yellow2); }
.log-line.info  .log-cat   { color: var(--blue2); }
.log-line.critical { background: rgba(239,68,68,.18); }
.log-line.critical .log-cat { color: #ff4444; }
...
.clock-font-card:hover, .clock-font-card.selected {
  border-color: var(--accent);
  background: rgba(99,102,241,.06);
}
.clock-font-card .fn-name  { font-size: .68rem; color: var(--text3); margin-bottom: 5px; }
.clock-font-card .fn-prev  { font-size: 1.1rem; color: var(--accent3); letter-spacing: 2px; }
...
.provider-key  { font-size: .75rem; color: var(--text3); font-family: monospace; margin: 4px 0; }
...
.health-card .h-title { font-weight: 700; margin-bottom: 6px; display: flex; align-items: center; gap: 6px; }
.health-card .h-status { font-size: .8rem; color: var(--text2); }
.health-card .h-last   { font-size: .7rem; color: var(--text3); margin-top: 6px; }
.health-card .h-action { margin-top: 8px; }

/* ── COMMAND TABLE ── */
.cmd-filter { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 14px; }
.cmd-tag {
  padding: 4px 12px;
  border-radius: 20px;
  font-size: .74rem;
...
.sr-type { width: 70px; font-size: .7rem; color: var(--text3); text-transform: uppercase; }
.sr-name { font-weight: 600; flex: 1; font-size: .84rem; }
.sr-desc { font-size: .76rem; color: var(--text2); }
...
.act-time { color: var(--text3); white-space: nowrap; font-family: monospace; }
.act-cat  { width: 70px; font-weight: 700; flex-shrink: 0; }
.act-cat.clock    { color: var(--blue2); }
.act-cat.crm      { color: var(--green2); }
.act-cat.security { color: var(--red2); }
.act-cat.sales    { color: var(--yellow2); }
.act-cat.ai       { color: var(--purple); }
.act-cat.system   { color: var(--text2); }
...
.text-success { color: var(--green2); }
.text-danger  { color: var(--red2); }
.text-accent  { color: var(--accent3); }
.refresh-tag  { font-size: .7rem; color: var(--text3); text-align: right; margin-top: 8px; }

/* ── RESPONSIVE ── */
@media (max-width: 900px) {
  .sidebar { width: 60px; }
  .sidebar-logo h1, .sidebar-logo .version, .nav-section, .nav-item span:not(.nav-icon), .nav-badge { display: none; }
...
_LCT_JS = """
// ── Real-time ping (WebSocket-like via polling) ──
let _pingHistory = [];
function measurePing() {
  ...
  fetch('/api/ping?t=' + t0)
    ...
    .then(d => {
      ...
      if (_pingHistory.length > 10) _pingHistory.shift();
      const avg = Math.round(_pingHistory.reduce((a,b)=>a+b,0)/_pingHistory.length);
      const el = document.getElementById('ping-display');
      if (el) {
        el.textContent = `PING ${ms}ms`;
        el.style.color = ms < 50 ? '#10b981' : ms < 150 ? '#f59e0b' : '#ef4444';
      }
    }).catch(() => {
      const el = document.getElementById('ping-display');
      if (el) { el.textContent = 'PING —'; el.style.color = '#64748b'; }
    });
}
measurePing();
...
function updateTopbarClock() {
  ...
  fetch('/api/time').then(r=>r.json()).then(d=>{
    el.textContent = d.date + ' | ' + d.time;
  }).catch(()=>{});
}
updateTopbarClock();
setInterval(updateTopbarClock, 5000);

// ── Live status dot ──
function updateLiveStatus() {
  fetch('/api/status').then(r=>r.json()).then(d=>{
    const el = document.getElementById('sidebar-online-dot');
    if (el) el.className = 'dot green pulse';
    const clk = document.getElementById('sidebar-clock-status');
    if (clk && d.clock) {
      clk.textContent = d.clock.state;
      clk.className = d.clock.is_running ? 'text-success' : 'text-danger';
    }
  }).catch(()=>{
    const el = document.getElementById('sidebar-online-dot');
...
function doGlobalSearch(q) {
  ...
  if (!q || q.length < 2) {
    const r = document.getElementById('search-results-box');
    if (r) r.innerHTML = '';
    return;
  }
    ...
    fetch('/api/search?q=' + encodeURIComponent(q))
      .then(r=>r.json()).then(d=>{
        ...
        if (!d.results || !d.results.length) {
          box.innerHTML = '<div class="search-result-item"><span class="text-muted">نتیجه‌ای نیافتیم</span></div>';
          return;
        }
        box.innerHTML = d.results.slice(0,12).map(r=>
...
function doAction(url, confirm_msg) {
  ...
  fetch(url).then(r=>r.json()).then(d=>{
    showToast(d.msg || d.error || 'Done', d.error ? 'error' : 'ok');
    setTimeout(()=>location.reload(), 800);
  }).catch(e => showToast('Error: '+e, 'error'));
}

// ── Save setting ──
function saveSetting(key, elId) {
  const el = document.getElementById(elId || ('s_'+key));
  if (!el) { showToast('Input not found', 'error'); return; }
  const val = el.value;
  fetch('/api/set?key='+encodeURIComponent(key)+'&val='+encodeURIComponent(val))
    .then(r=>r.json()).then(d=>{
      showToast(d.msg || d.error, d.error ? 'error' : 'ok');
...
function showToast(msg, type) {
  ...
  toast.style.cssText = `background:${type==='error'?'#450a0a':'#14532d'};color:${type==='error'?'#fca5a5':'#86efac'};
    padding:10px 16px;border-radius:8px;font-size:.82rem;max-width:280px;
    animation:fadeSlide .3s ease;border:1px solid ${type==='error'?'rgba(239,68,68,.3)':'rgba(16,185,129,.3)'};`;
...
function refreshActivity() {
  fetch('/api/activity').then(r=>r.json()).then(d=>{
    ...
    if (!feed || !d.items) return;
    feed.innerHTML = d.items.map(item=>
      `<div class="activity-item">
        <span class="act-time">${item.ts}</span>
        <span class="act-cat ${item.cat.toLowerCase()}">${item.cat}</span>
...
// ── AI Connection Test ──
function testAiConnection(pid) {
  const btn = document.getElementById('test-btn-'+pid);
  if (btn) { btn.textContent = '⏳ Testing...'; btn.disabled = true; }
  const resultEl = document.getElementById('test-result-'+pid);
  if (resultEl) resultEl.innerHTML = '';
  const t0 = performance.now();
  fetch('/api/ai_test?id='+pid)
    .then(r=>r.json()).then(d=>{
      const ms = Math.round(performance.now()-t0);
      if (btn) { btn.textContent = '🔌 Test'; btn.disabled = false; }
      if (resultEl) {
        if (d.ok) {
          resultEl.innerHTML = `<span class="badge green">🟢 Connected — ${ms}ms</span>`;
        } else {
          resultEl.innerHTML = `<span class="badge red">🔴 Failed: ${d.error||'Unknown'}</span>`;
        }
      }
    }).catch(e=>{
      if (btn) { btn.textContent = '🔌 Test'; btn.disabled = false; }
      if (resultEl) resultEl.innerHTML = `<span class="badge red">🔴 ${e}</span>`;
    });
}

// ── Feature filter ──
function filterFeatures(cat) {
  document.querySelectorAll('.cmd-tag').forEach(t=>t.classList.remove('active'));
...
def _lct_sidebar(active_page: str = "") -> str:
    nav_items = [
        ("/",          "📊", "Dashboard",    "dashboard"),
        ("/features",  "⚙️", "Features",     "features"),
        ("/commands",  "⌨️", "Commands",     "commands"),
        ("/clock",     "⏰", "Clock",        "clock"),
        ("/ai",        "🤖", "AI Center",    "ai"),
    ...
    ck = _lct_get_clock_real_status()
    ck_dot_cls = "green pulse" if ck["is_running"] else ("yellow" if ck["state"] == "STARTING" else "red")

    h = f'<aside class="sidebar">'
    h += f'<div class="sidebar-logo"><h1>💎 Onyx Self</h1><div class="version">{LCT_VERSION} — Control Tower</div></div>'
    h += f'<div class="sidebar-status"><span class="dot green pulse" id="sidebar-online-dot"></span> Online</div>'
    h += f'<div class="sidebar-status"><span class="dot {ck_dot_cls}"></span> Clock: <span id="sidebar-clock-status" class="{"text-success" if ck["is_running"] else "text-danger"}">{ck["state"]}</span></div>'
    h += '<div class="nav-section">Navigation</div>'
    for href, icon, label, page_id in nav_items:
        active_cls = "active" if active_page == page_id else ""
        # Add error badge to logs
        badge = ""
        if page_id == "logs":
            try:
                with _db_lock:
                    conn = get_conn()
                    errs = conn.execute("SELECT COUNT(*) FROM black_box WHERE level IN ('error','critical') AND ts LIKE ?", (f"{jalali()}%",)).fetchone()[0]
                if errs:
                    badge = f'<span class="nav-badge red">{errs}</span>'
            except Exception:
                pass
        h += f'<a href="{href}" class="nav-item {active_cls}"><span class="nav-icon">{icon}</span><span>{label}</span>{badge}</a>'
    h += '</aside>'
    return h

def _lct_topbar(title: str, subtitle: str = "") -> str:
    now = iran_now()
    return f'''<div class="topbar">
  <div class="topbar-title">{_html.escape(title)}{f'<span class="text-muted" style="font-weight:400;font-size:.8rem;margin-right:10px"> — {_html.escape(subtitle)}</span>' if subtitle else ''}</div>
  <div class="search-box" style="max-width:240px">
    <span class="search-icon">🔍</span>
    <input type="text" placeholder="جستجوی سراسری..." oninput="doGlobalSearch(this.value)" id="global-search-input">
  </div>
  <div id="ping-display">PING —</div>
  <div class="topbar-time" id="topbar-time">{jalali(now)} | {now.strftime("%H:%M:%S")}</div>
</div>
<div id="search-results-box" class="search-results" style="position:fixed;top:56px;right:80px;width:380px;z-index:500;display:none"></div>
<script>
document.getElementById('global-search-input').addEventListener('input', function(){{
  const box = document.getElementById('search-results-box');
  box.style.display = this.value.length > 1 ? 'block' : 'none';
  doGlobalSearch(this.value);
}});
document.addEventListener('click', function(e){{
  if (!e.target.closest('.search-box') && !e.target.closest('#search-results-box')) {{
    const box = document.getElementById('search-results-box');
...
def _lct_page_wrap(active: str, title: str, subtitle: str, content: str) -> str:
    return f"""<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>💎 Onyx Self {LCT_VERSION} — {_html.escape(title)}</title>
<style>{_LCT_CSS}</style>
</head>
<body>
{_lct_sidebar(active)}
<div class="main">
{_lct_topbar(title, subtitle)}
<div class="page">
{content}
</div>
<div style="padding:12px 24px;border-top:1px solid var(--border);font-size:.7rem;color:var(--text3);background:var(--bg2)">
  {_html.escape(WATERMARK)} | Local Control Tower — localhost only | {jalali()} {iran_now().strftime('%H:%M')}
</div>
</div>
<script>{_LCT_JS}</script>
</body>
</html>"""

def _lct_badge(label: str, color: str) -> str:
    return f'<span class="badge {color}">{_html.escape(str(label))}</span>'

def _lct_dot_html(color: str, pulse: bool = False) -> str:
    cls = f"dot {color}" + (" pulse" if pulse else "")
    return f'<span class="{cls}"></span>'

# ══════════════════════════════════════════════════════
#  PAGE: DASHBOARD
# ══════════════════════════════════════════════════════
def _lct_page_dashboard() -> str:
    d = {}
    try:
        with _db_lock:
            conn = get_conn()
            d["customers"]     = conn.execute("SELECT COUNT(*) FROM crm_customers").fetchone()[0]
            d["vip"]           = conn.execute("SELECT COUNT(*) FROM crm_customers WHERE vip_level>0").fetchone()[0]
            d["pending"]       = conn.execute("SELECT COUNT(*) FROM store_orders WHERE status='pending'").fetchone()[0]
            d["total_orders"]  = conn.execute("SELECT COUNT(*) FROM store_orders").fetchone()[0]
            d["revenue"]       = conn.execute("SELECT COALESCE(SUM(price),0) FROM store_orders WHERE status='approved'").fetchone()[0]
            d["products"]      = conn.execute("SELECT COUNT(*) FROM store_products WHERE active=1").fetchone()[0]
            d["tickets"]       = conn.execute("SELECT COUNT(*) FROM support_tickets WHERE status='open'").fetchone()[0]
            d["total_cmds"]    = conn.execute("SELECT COUNT(*) FROM cmd_history").fetchone()[0]
            d["errors_today"]  = conn.execute("SELECT COUNT(*) FROM black_box WHERE level IN ('error','critical') AND ts LIKE ?", (f"{jalali()}%",)).fetchone()[0]
            d["contacts"]      = conn.execute("SELECT COUNT(*) FROM contacts").fetchone()[0]
            d["recent_logs"]   = conn.execute("SELECT * FROM black_box WHERE level IN ('error','critical','warning') ORDER BY id DESC LIMIT 6").fetchall()
    except Exception as e:
        d.setdefault("err", str(e))

    ck   = _lct_get_clock_real_status()
    sys  = _lct_get_sys_metrics()
    now  = iran_now()

    # Alerts
    alerts = []
    if d.get("pending",0) > 0:
        alerts.append(("warn", f"📦 {d['pending']} سفارش در انتظار تأیید"))
    if d.get("errors_today",0) > 0:
        alerts.append(("danger", f"❌ {d['errors_today']} خطا امروز"))
    if _airlock_v9:
        alerts.append(("danger", "🔒 Airlock Mode فعال است"))
    if ck["state"] == "FAILED":
        alerts.append(("danger", f"⏰ Clock FAILED: {ck.get('fail_reason','')[:60]}"))

    h = ""
    for cls, msg in alerts:
        h += f'<div class="alert {cls}"><span>{_html.escape(msg)}</span></div>'
    if not alerts:
        h += '<div class="alert ok">✅ همه سیستم‌ها طبیعی هستند</div>'

    # Stats grid
    stats = [
        ("👥 مشتریان",     d.get("customers",0),    f'VIP: {d.get("vip",0)}',       "blue"),
        ("📦 سفارشات",     d.get("total_orders",0), f'Pending: {d.get("pending",0)}', "yellow" if d.get("pending") else "green"),
        ("💰 درآمد",       f'{d.get("revenue",0):,}', "تومان",                       "green"),
        ("🛍 محصولات",     d.get("products",0),     "فعال",                          "blue"),
        ("🎫 تیکت باز",   d.get("tickets",0),      "",                              "yellow" if d.get("tickets") else "green"),
        ("⚡ دستورات",     d.get("total_cmds",0),   "اجرا‌شده",                     "blue"),
        ("📒 مخاطبان",    d.get("contacts",0),     "ذخیره",                         "blue"),
        ("❌ خطا امروز",   d.get("errors_today",0), "Black Box",                     "red" if d.get("errors_today") else "green"),
    ]
    h += '<div class="stats-grid">'
    for label, val, sub, color in stats:
        h += f'''<div class="stat-card {color}">
  <div class="label">{_html.escape(label)}</div>
  <div class="value">{_html.escape(str(val))}</div>
  <div class="sub">{_html.escape(str(sub))}</div>
</div>'''
    h += '</div>'
    ...
    # System Status
    ck_dot  = "green pulse" if ck["is_running"] else ("yellow" if ck["state"]=="STARTING" else "red")
    h += f'''<div class="card">
  <div class="card-header">🔴 Real-Time Status</div>
  <div class="card-body status-list">
    <div class="status-row"><span class="status-label">Selfbot Session</span><span class="status-val"><span class="dot green pulse"></span> CONNECTED</span></div>
    <div class="status-row"><span class="status-label">Clock Service</span><span class="status-val"><span class="dot {ck_dot}"></span> {_html.escape(ck["state"])}</span></div>
    <div class="status-row"><span class="status-label">Airlock Mode</span><span class="status-val">{"<span class='dot yellow'></span> ACTIVE 🔒" if _airlock_v9 else "<span class='dot green'></span> OFF"}</span></div>
    <div class="status-row"><span class="status-label">Black Box</span><span class="status-val"><span class="dot green"></span> ENABLED</span></div>
    <div class="status-row"><span class="status-label">Self-Healing</span><span class="status-val"><span class="dot green"></span> ENABLED</span></div>
    <div class="status-row"><span class="status-label">Local Tower</span><span class="status-val"><span class="dot green pulse"></span> RUNNING :{LCT_PORT}</span></div>
    <div class="status-row"><span class="status-label">Clock Last Tick</span><span class="status-val">{"never" if ck["last_tick_ago"]<0 else f"{ck['last_tick_ago']}s ago"}</span></div>
  </div>
</div>'''

    # System Metrics
    cpu_color = "var(--green)" if sys["cpu"] < 60 else ("var(--yellow)" if sys["cpu"] < 85 else "var(--red)")
    ram_color = "var(--green)" if sys["ram"] < 70 else ("var(--yellow)" if sys["ram"] < 90 else "var(--red)")
    h += f'''<div class="card">
  <div class="card-header">💻 System Metrics</div>
  <div class="card-body status-list">
    <div class="status-row"><span class="status-label">CPU</span><span class="status-val" style="color:{cpu_color}">{sys["cpu"]:.1f}%</span></div>
    <div class="status-row"><span class="status-label">RAM</span><span class="status-val" style="color:{ram_color}">{sys["ram"]:.1f}% ({sys["ram_mb"]} MB)</span></div>
    <div class="status-row"><span class="status-label">Uptime</span><span class="status-val">{sys["uptime"]}</span></div>
    <div class="status-row"><span class="status-label">Version</span><span class="status-val text-accent">{_html.escape(PROJECT)}</span></div>
    <div class="status-row"><span class="status-label">Database</span><span class="status-val text-success">Connected</span></div>
    <div class="status-row"><span class="status-label">Date</span><span class="status-val">{jalali(now)}</span></div>
    <div class="status-row"><span class="status-label">Time</span><span class="status-val monospace">{now.strftime("%H:%M:%S")}</span></div>
  </div>
</div>'''
    h += '</div>'

    # Recent logs + Live Activity
    h += '<div class="two-col">'
    h += '<div class="card"><div class="card-header">❌ خطاهای اخیر</div><div class="card-body">'
    if d.get("recent_logs"):
        h += '<div class="log-container">'
        for r in d["recent_logs"]:
            lv = str(r["level"]) if "level" in r.keys() else "info"
            h += f'<div class="log-line {lv}"><span class="log-ts">{str(r.get("ts",""))[:16]}</span><span class="log-cat">[{lv.upper()}]</span><span class="log-msg">{_html.escape(str(r.get("event",""))[:40])}: {_html.escape(str(r.get("detail",""))[:50])}</span></div>'
        h += '</div>'
    else:
        h += '<div class="alert ok" style="margin:0">✅ بدون خطا</div>'
    h += '<div class="refresh-tag"><a href="/logs">مشاهده همه →</a></div></div></div>'

    h += '<div class="card"><div class="card-header">📡 Live Activity</div><div class="card-body"><div class="activity-feed" id="activity-feed"></div></div></div>'
    h += '</div>'

    h += '<script>setTimeout(()=>location.reload(),30000)</script>'
    h += '<p class="refresh-tag">🔄 Auto-refresh هر ۳۰ ثانیه</p>'
    return _lct_page_wrap("dashboard", "Dashboard", f"Onyx Self {LCT_VERSION}", h)

# ══════════════════════════════════════════════════════
#  PAGE: FEATURES
# ══════════════════════════════════════════════════════
def _lct_page_features() -> str:
    registry = _lct_build_feature_registry()
    cats = sorted(set(r["category"] for r in registry))
    total_cmds = sum(len(v) for v in FULL_MENU.values())

    # extra standalone features
    extra_features = [
        {"id":"clock_svc",     "name":"Clock Service",       "icon":"⏰","category":"clock",      "desc":"ساعت ایران در نام پروفایل — State Machine V10"},
        {"id":"airlock",       "name":"Airlock Mode",        "icon":"🔒","category":"security",   "desc":"حالت قرنطینه اضطراری"},
        {"id":"self_healing",  "name":"Self-Healing Engine", "icon":"🛡️","category":"system",    "desc":"تشخیص خطا و خودترمیم"},
        {"id":"black_box",     "name":"Black Box Recorder",  "icon":"📦","category":"system",    "desc":"ثبت تمام رویدادهای سیستم"},
        {"id":"product_know",  "name":"Product Knowledge",   "icon":"📚","category":"sales",     "desc":"Context هوشمند برای هر محصول"},
        {"id":"smart_neg",     "name":"Smart Negotiator",    "icon":"💼","category":"sales",     "desc":"مذاکره هوشمند قیمت"},
        {"id":"honeytrap",     "name":"Honeytrap Detector",  "icon":"🍯","category":"security",  "desc":"تشخیص رفتار مشکوک و Scam"},
        {"id":"watermark",     "name":"Watermark Engine",    "icon":"💧","category":"sales",     "desc":"واترمارک نامرئی روی فایل‌ها"},
        {"id":"link_intel",    "name":"Link Intelligence",   "icon":"🔗","category":"tools",     "desc":"تحلیل هوشمند لینک"},
        {"id":"exporter",      "name":"Universal Exporter",  "icon":"📤","category":"tools",     "desc":"اکسپورت داده‌ها به JSON/CSV/TXT"},
        {"id":"radar",         "name":"Personal Radar",      "icon":"📡","category":"system",    "desc":"هشدارهای مهم و رویداد"},
        {"id":"identity",      "name":"Identity Verifier",   "icon":"🔍","category":"security",  "desc":"بررسی هویت و ریسک کاربران"},
        {"id":"event_replay",  "name":"Event Replay",        "icon":"🎬","category":"sales",     "desc":"بازسازی تاریخچه رویدادها"},
        {"id":"ctx_switch",    "name":"Context Switcher",    "icon":"🔄","category":"ai",        "desc":"سوییچ Context هوشمند"},
        {"id":"cmd_studio",    "name":"Command Studio",      "icon":"⌨️","category":"tools",    "desc":"ساخت دستور سفارشی"},
        {"id":"bl_intel",      "name":"Blacklist Intelligence","icon":"🚦","category":"security","desc":"سطح‌بندی ریسک ۵-level"},
        {"id":"ctrl_tower_tg", "name":"Control Tower (TG)",  "icon":"🏰","category":"system",   "desc":"مرکز کنترل تلگرام: برج_کنترل"},
        {"id":"local_tower",   "name":"Local Control Tower", "icon":"🖥️","category":"system",  "desc":f"این پنل — localhost:{LCT_PORT}"},
    ]

    h = '<div class="page-header">'
    h += f'<h2>⚙️ Feature Manager</h2>'
    h += f'<div class="sub">{len(registry) + len(extra_features)} دسته | {total_cmds} دستور کل</div>'
    h += '</div>'

    # Category filter tabs
    h += '<div class="cmd-filter">'
    h += '<span class="cmd-tag active" onclick="filterFeatures(\'all\')">همه</span>'
    for cat in cats:
        h += f'<span class="cmd-tag" onclick="filterFeatures(\'{cat}\')">{cat}</span>'
    for xcat in sorted(set(f["category"] for f in extra_features)):
        if xcat not in cats:
            h += f'<span class="cmd-tag" onclick="filterFeatures(\'{xcat}\')">{xcat}</span>'
    h += '</div>'
    ...
    for feat in registry:
        fid      = feat["id"]
        disabled = _lct_is_feature_disabled(fid)
        card_cls = "feature-card disabled" if disabled else "feature-card"
        badge    = _lct_badge("DISABLED","red") if disabled else _lct_badge("ACTIVE","green")
        cmds_preview = ", ".join(c[0] for c in feat["commands"][:4])
        if len(feat["commands"]) > 4:
            cmds_preview += f" +{len(feat['commands'])-4}"
        h += f'''<div class="{card_cls}" data-cat="{feat["category"]}">
  <div class="fc-top">
    <div class="fc-name">{feat["icon"]} {_html.escape(feat["name"])}</div>
    {badge}
  </div>
  <div class="fc-desc">{_html.escape(feat["name_fa"])} — {feat["cmd_count"]} دستور</div>
  <div class="fc-meta">دستورات: {_html.escape(cmds_preview)}</div>
  <div class="fc-actions">
    {"" if disabled else f'<a class="btn btn-danger btn-sm" href="/api/feature?id={fid}&action=disable">Disable</a>'}
    {f'<a class="btn btn-success btn-sm" href="/api/feature?id={fid}&action=enable">Enable</a>' if disabled else ""}
    <a class="btn btn-ghost btn-sm" href="/commands?cat={feat["category"]}">Commands</a>
  </div>
</div>'''
    ...
    for feat in extra_features:
        fid = feat["id"]
        # Special real-time status
        if fid == "clock_svc":
            ck = _lct_get_clock_real_status()
            state = ck["state"]
            col   = "green" if state=="RUNNING" else ("yellow" if state=="STARTING" else "red")
            badge = _lct_badge(state, col)
            actions = '<a class="btn btn-success btn-sm" href="/api/clock?action=start">Start</a><a class="btn btn-danger btn-sm" href="/api/clock?action=stop">Stop</a><a class="btn btn-ghost btn-sm" href="/clock">Details</a>'
        elif fid == "airlock":
            badge   = _lct_badge("ACTIVE 🔒","yellow") if _airlock_v9 else _lct_badge("OFF","gray")
            actions = '<a class="btn btn-danger btn-sm" href="/api/airlock?action=on">Activate</a><a class="btn btn-success btn-sm" href="/api/airlock?action=off">Deactivate</a>'
        else:
            disabled = _lct_is_feature_disabled(fid)
            badge    = _lct_badge("DISABLED","red") if disabled else _lct_badge("ACTIVE","green")
            en_btn   = f'<a class="btn btn-success btn-sm" href="/api/feature?id={fid}&action=enable">Enable</a>' if disabled else ""
            dis_btn  = f'<a class="btn btn-danger btn-sm" href="/api/feature?id={fid}&action=disable">Disable</a>' if not disabled else ""
            actions  = en_btn + dis_btn
        h += f'''<div class="feature-card" data-cat="{feat["category"]}">
  <div class="fc-top">
    <div class="fc-name">{feat["icon"]} {_html.escape(feat["name"])}</div>
    {badge}
  </div>
  <div class="fc-desc">{_html.escape(feat["desc"])}</div>
  <div class="fc-actions">{actions}</div>
</div>'''
    h += '</div>'
    return _lct_page_wrap("features", "Feature Manager", "Enable · Disable · Status", h)

# ══════════════════════════════════════════════════════
#  PAGE: COMMANDS
# ══════════════════════════════════════════════════════
def _lct_page_commands(cat_filter: str = "") -> str:
    # Collect all commands from FULL_MENU
    all_cmds = []
    for cat_fa, cmds in FULL_MENU.items():
        for cmd in cmds:
            name, desc, usage, example = cmd[0], cmd[1], cmd[2] if len(cmd)>2 else "", cmd[3] if len(cmd)>3 else ""
            all_cmds.append({
                "name": name, "desc": desc, "usage": usage, "example": example,
                "cat": cat_fa,
            })

    cats = sorted(set(c["cat"] for c in all_cmds))
    total = len(all_cmds)

    h = f'<div class="page-header"><h2>⌨️ Command Center</h2><div class="sub">{total} دستور در {len(cats)} دسته</div></div>'
    h += '<div style="display:flex;gap:10px;margin-bottom:14px;align-items:center">'
    h += '<input type="text" class="form-control" style="max-width:300px" placeholder="جستجو در دستورات..." oninput="filterCommands(this.value)">'
    h += f'<span class="text-muted" style="font-size:.8rem">{total} دستور</span>'
    h += '</div>'

    # Category tabs
    h += '<div class="cmd-filter" style="margin-bottom:14px">'
    h += f'<a class="cmd-tag {"active" if not cat_filter else ""}" href="/commands">همه</a>'
    for cat in cats:
        active_cls = "active" if cat == cat_filter else ""
        h += f'<a class="cmd-tag {active_cls}" href="/commands?cat={_html.escape(cat)}">{_html.escape(cat)}</a>'
    h += '</div>'

    # Table
    filtered = [c for c in all_cmds if not cat_filter or c["cat"] == cat_filter]
    h += '<div class="card"><div class="card-body" style="padding:0">'
    h += '<table class="table">'
    h += '<tr><th>دستور</th><th>توضیح</th><th>نحوه استفاده</th><th>مثال</th><th>دسته</th></tr>'
    for cmd in filtered:
        h += f'<tr class="cmd-row">'
        h += f'<td><code style="color:var(--accent3);background:rgba(99,102,241,.1);padding:2px 6px;border-radius:4px">{_html.escape(cmd["name"])}</code></td>'
        h += f'<td>{_html.escape(cmd["desc"])}</td>'
        h += f'<td><code style="font-size:.72rem;color:var(--text2)">{_html.escape(cmd["usage"])}</code></td>'
        h += f'<td><code style="font-size:.72rem;color:var(--green2)">{_html.escape(cmd["example"])}</code></td>'
        h += f'<td><span class="badge gray" style="font-size:.66rem">{_html.escape(cmd["cat"][:20])}</span></td>'
        h += '</tr>'
    h += '</table></div></div>'
    return _lct_page_wrap("commands", "Command Center", f"{len(filtered)} دستور", h)

# ══════════════════════════════════════════════════════
#  PAGE: CLOCK
# ══════════════════════════════════════════════════════
def _lct_page_clock() -> str:
    ck = _lct_get_clock_real_status()
    state = ck["state"]
    now_t = iran_now().strftime("%H:%M")
    cur_font = ck.get("font","normal")
    STATE_COLOR = {"RUNNING":"green","STARTING":"yellow","FAILED":"red","STOPPING":"yellow","DISABLED":"gray"}
    col = STATE_COLOR.get(state,"gray")

    h = '<div class="page-header"><h2>⏰ Clock Service</h2></div>'

    # State card
    h += '<div class="two-col">'
    h += f'''<div class="card">
  <div class="card-header">⏰ وضعیت فعلی</div>
  <div class="card-body">
    <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">
      <span style="font-size:3rem">⏰</span>
      <div>
        <div style="font-size:1.6rem;font-weight:800">{_lct_dot_html(col,col=="green")} {state}</div>
        {"<div class='text-danger' style='font-size:.8rem'>"+_html.escape(ck["fail_reason"])+"</div>" if ck.get("fail_reason") else ""}
        <div class="text-muted" style="font-size:.76rem;margin-top:4px">Last tick: {"never" if ck["last_tick_ago"]<0 else f"{ck['last_tick_ago']}s ago"}</div>
        <div class="text-muted" style="font-size:.76rem">Saved in DB: {"✅" if ck.get("saved_active") else "❌"} | Font: {_html.escape(cur_font)}</div>
      </div>
    </div>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      <a class="btn btn-success" href="/api/clock?action=start">▶ Start</a>
      <a class="btn btn-danger" href="/api/clock?action=stop">■ Stop</a>
      <a class="btn btn-ghost" href="/api/clock?action=restart">↺ Restart</a>
      <a class="btn btn-ghost" href="/api/clock?action=healthcheck">🩺 Health Check</a>
    </div>
  </div>
</div>'''

    # State machine diagram
    states = ["DISABLED","STARTING","RUNNING","FAILED","STOPPING"]
    h += f'<div class="card"><div class="card-header">📊 State Machine</div><div class="card-body">'
    h += '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">'
    for s in states:
        c = STATE_COLOR.get(s,"gray")
        weight = "800" if s == state else "400"
        h += f'<span class="badge {c}" style="font-weight:{weight};transform:{f"scale(1.15)" if s==state else "scale(1)"}">{s}</span>'
    h += '</div>'
    h += '<div style="font-size:.74rem;color:var(--text3);line-height:2">DISABLED → START → STARTING → RUNNING<br>RUNNING → (error×5) → FAILED → (Health Check) → STARTING<br>Any state → (cancel) → STOPPING → DISABLED</div>'
    h += '</div></div>'
    h += '</div>'

    # Font picker
    h += '<div class="card"><div class="card-header">🔤 Font Picker</div><div class="card-body">'
    try:
        preview_now = apply_clock_font(now_t, cur_font)
    except Exception:
        preview_now = now_t
    h += f'<div class="clock-display">{_html.escape(preview_now)}</div>'
    h += '<div class="clock-font-grid">'
    for fn in CLOCK_FONTS:
        try:
            prev = apply_clock_font(now_t, fn)
        except Exception:
            prev = now_t
        sel = "selected" if fn == cur_font else ""
        h += f'''<div class="clock-font-card {sel}" onclick="doAction('/api/set?key=clock_font&val={_html.escape(fn)}')">
  <div class="fn-name">{_html.escape(fn)}</div>
  <div class="fn-prev">{_html.escape(prev[:12])}</div>
</div>'''
    h += '</div>'
    h += '<div class="refresh-tag" style="margin-top:10px">کلیک روی هر فونت برای انتخاب</div>'
    h += '</div></div>'

    # Clock suffix
    cur_sfx = setting("clock_suffix","")
    h += f'''<div class="card"><div class="card-header">🏷 Clock Suffix</div><div class="card-body">
  <div class="form-group">
    <label class="form-label">متن اضافی کنار ساعت (اختیاری)</label>
    <input type="text" id="s_clock_suffix" class="form-control" value="{_html.escape(cur_sfx)}" placeholder="مثال: | 🌙">
  </div>
  <button class="btn btn-primary btn-sm" onclick="saveSetting('clock_suffix')">ذخیره</button>
</div></div>'''

    h += '<script>setTimeout(()=>location.reload(),15000)</script>'
    return _lct_page_wrap("clock", "Clock Service", "State Machine · Health · Fonts", h)

# ══════════════════════════════════════════════════════
#  PAGE: AI CENTER
# ══════════════════════════════════════════════════════
def _lct_page_ai() -> str:
    providers = _lct_get_ai_providers()
    # AI stats
    ai_req_total    = int(setting("ai_req_total","0"))
    ai_req_ok       = int(setting("ai_req_ok","0"))
    ai_req_fail     = int(setting("ai_req_fail","0"))
    ai_last_req     = setting("ai_last_req","—")
    ai_max_req      = setting("ai_max_req","100")
    ai_ctx_limit    = setting("ai_ctx_limit","4000")
    ai_timeout      = setting("ai_timeout","30")
    ai_cache        = setting("ai_cache","1")
    ai_enabled      = setting("ai_enabled","1")

    PROVIDER_ICONS = {"gemini":"🔮","openai":"🤖","anthropic":"🧠","local":"💻","custom":"⚙️"}
    PROVIDER_NAMES = {"gemini":"Gemini (Google)","openai":"OpenAI (GPT)","anthropic":"Claude (Anthropic)","local":"Local Model","custom":"Custom API"}

    h = '<div class="page-header"><h2>🤖 AI API Center</h2><div class="sub">مدیریت Provider، API Key، Model و مصرف</div></div>'

    # AI Enable/Disable
    ai_on = ai_enabled == "1"
    h += f'''<div class="alert {"ok" if ai_on else "warn"}">
  {"✅ AI فعال است" if ai_on else "⚠️ AI غیرفعال است"}
  <a class="btn btn-{"danger" if ai_on else "success"} btn-sm" style="margin-right:auto" href="/api/ai_toggle">{"Disable AI" if ai_on else "Enable AI"}</a>
</div>'''

    # Usage stats
    h += '<div class="stats-grid" style="grid-template-columns:repeat(auto-fill,minmax(150px,1fr));margin-bottom:20px">'
    for lbl, val, col in [
        ("📤 کل درخواست", ai_req_total, "blue"),
        ("✅ موفق",        ai_req_ok,   "green"),
        ("❌ ناموفق",     ai_req_fail, "red" if ai_req_fail else "green"),
        ("آخرین درخواست", ai_last_req[:10] if ai_last_req!="—" else "—", "blue"),
    ]:
        h += f'<div class="stat-card {col}"><div class="label">{_html.escape(str(lbl))}</div><div class="value" style="font-size:1.1rem">{_html.escape(str(val))}</div></div>'
    h += '</div>'

    # Providers
    h += '<div class="card"><div class="card-header">🔌 AI Providers</div><div class="card-body">'
    if providers:
        for p in providers:
            pid      = p.get("id",0)
            ptype    = p.get("provider_type","custom")
            pname    = PROVIDER_NAMES.get(ptype, p.get("provider_name","Unknown"))
            icon     = PROVIDER_ICONS.get(ptype,"⚙️")
            is_def   = bool(p.get("is_default",0))
            is_en    = bool(p.get("enabled",1))
            api_key  = p.get("api_key","")
            model    = p.get("model","")
            h += f'''<div class="provider-card">
  <div class="provider-icon">{icon}</div>
  <div class="provider-info">
    <div class="provider-name">{_html.escape(pname)} {"⭐" if is_def else ""}</div>
    <div class="provider-key">API Key: {_lct_mask_key(api_key)}</div>
    <div style="font-size:.76rem;color:var(--text2)">Model: {_html.escape(model or "—")} | {"✅ Enabled" if is_en else "❌ Disabled"}</div>
    <div class="provider-actions">
      <button class="btn btn-ghost btn-sm" id="test-btn-{pid}" onclick="testAiConnection({pid})">🔌 Test</button>
      <a class="btn btn-ghost btn-sm" href="/api/ai_default?id={pid}">⭐ Set Default</a>
      <a class="btn {"btn-danger" if is_en else "btn-success"} btn-sm" href="/api/ai_toggle_provider?id={pid}">{"Disable" if is_en else "Enable"}</a>
      <a class="btn btn-danger btn-sm" href="/api/ai_delete?id={pid}" onclick="return confirm('حذف کنم؟')">Delete</a>
      <span id="test-result-{pid}"></span>
    </div>
  </div>
</div>'''
    else:
        h += '<div class="alert info">هیچ Providerی تنظیم نشده. از فرم زیر اضافه کن.</div>'
    h += '</div></div>'

    # Add Provider form
    h += '''<div class="card"><div class="card-header">➕ Add Provider</div><div class="card-body">
  <div class="two-col">
    <div class="form-group">
      <label class="form-label">Provider</label>
      <select id="new_provider_type" class="form-control">
    ...
    <div class="form-group">
      ...
      <input type="text" id="new_model" class="form-control" placeholder="gemini-1.5-flash / gpt-4o / ...">
    </div>
  </div>
  <div class="form-group">
    <label class="form-label">API Key (ذخیره‌شده رمزنگاری‌شده)</label>
    <input type="password" id="new_api_key" class="form-control" placeholder="sk-... یا AI...">
  </div>
  <div class="form-group">
    <label class="form-label">Base URL (اختیاری — برای OpenAI-compatible)</label>
    <input type="text" id="new_base_url" class="form-control" placeholder="https://api.openai.com/v1">
  </div>
  <button class="btn btn-primary" onclick="addAiProvider()">➕ Add Provider</button>
</div></div>
<script>
function addAiProvider() {
  const data = {
    type: document.getElementById('new_provider_type').value,
    model: document.getElementById('new_model').value,
    key: document.getElementById('new_api_key').value,
    url: document.getElementById('new_base_url').value,
  };
  if (!data.key) { showToast('API Key الزامی است','error'); return; }
  fetch('/api/ai_add', {
    method:'POST',
    ...
    showToast(d.msg||d.error, d.error?'error':'ok');
    if (!d.error) setTimeout(()=>location.reload(),1000);
  }).catch(e=>showToast(''+e,'error'));
}
</script>'''

    # AI Settings
    h += f'''<div class="card"><div class="card-header">⚙️ AI Usage Control</div><div class="card-body">
  <div class="three-col">
    <div class="form-group">
      <label class="form-label">Max Requests (روزانه)</label>
      <input type="number" id="s_ai_max_req" class="form-control" value="{_html.escape(ai_max_req)}">
      <button class="btn btn-ghost btn-sm" style="margin-top:6px" onclick="saveSetting('ai_max_req')">ذخیره</button>
    </div>
    <div class="form-group">
      <label class="form-label">Context Limit (توکن)</label>
      <input type="number" id="s_ai_ctx_limit" class="form-control" value="{_html.escape(ai_ctx_limit)}">
      <button class="btn btn-ghost btn-sm" style="margin-top:6px" onclick="saveSetting('ai_ctx_limit')">ذخیره</button>
    </div>
    <div class="form-group">
      <label class="form-label">Timeout (ثانیه)</label>
      <input type="number" id="s_ai_timeout" class="form-control" value="{_html.escape(ai_timeout)}">
      <button class="btn btn-ghost btn-sm" style="margin-top:6px" onclick="saveSetting('ai_timeout')">ذخیره</button>
    </div>
  </div>
  <div class="form-group">
    <label class="form-label">Cache پاسخ‌ها</label>
    <select id="s_ai_cache" class="form-control" style="max-width:200px">
      <option value="1" {"selected" if ai_cache=="1" else ""}>فعال</option>
      <option value="0" {"selected" if ai_cache=="0" else ""}>غیرفعال</option>
    </select>
    <button class="btn btn-ghost btn-sm" style="margin-top:6px" onclick="saveSetting('ai_cache')">ذخیره</button>
  </div>
</div></div>'''

    # AI Feature Mapping
    ai_features = [
        ("smart_neg",  "Smart Negotiator",    "gemini"),
        ("ctx_switch", "Context Switcher",    "local"),
        ("honeytrap",  "Honeytrap Detector",  "local"),
        ("product_know","Product Knowledge",  "gemini"),
        ("identity",   "Identity Verifier",   "local"),
        ("link_intel", "Link Intelligence",   "local"),
    ]
    h += '<div class="card"><div class="card-header">🗺 AI Feature Mapping</div><div class="card-body">'
    h += '<table class="table"><tr><th>Feature</th><th>Provider</th><th>Model</th></tr>'
    for fid, fname, default_prov in ai_features:
        prov_key = f"ai_feat_provider_{fid}"
        model_key= f"ai_feat_model_{fid}"
        cur_prov = setting(prov_key, default_prov)
        cur_mod  = setting(model_key, "")
        h += f'''<tr>
  <td><strong>{_html.escape(fname)}</strong></td>
  <td>
    <select class="form-control" style="max-width:160px" id="s_{prov_key}" onchange="saveSetting('{prov_key}')">
      <option value="local" {"selected" if cur_prov=="local" else ""}>Local Rules</option>
      <option value="gemini" {"selected" if cur_prov=="gemini" else ""}>Gemini</option>
      <option value="openai" {"selected" if cur_prov=="openai" else ""}>OpenAI</option>
      <option value="anthropic" {"selected" if cur_prov=="anthropic" else ""}>Claude</option>
    </select>
  </td>
  <td><input type="text" class="form-control" style="max-width:160px" id="s_{model_key}"
    value="{_html.escape(cur_mod)}" placeholder="پیش‌فرض Provider"
    onblur="saveSetting('{model_key}')"></td>
</tr>'''
    h += '</table></div></div>'
    return _lct_page_wrap("ai", "AI API Center", "Provider · Key · Model · Usage", h)

# ══════════════════════════════════════════════════════
#  PAGE: HEALTH
# ══════════════════════════════════════════════════════
def _lct_page_health() -> str:
    ck = _lct_get_clock_real_status()
    sys = _lct_get_sys_metrics()

    def health_status(ok: bool, warn: bool = False) -> tuple:
        if ok:    return "🟢 Healthy", "green"
        if warn:  return "🟡 Warning", "yellow"
        return    "🔴 Failed",   "red"

    # Clock health
    clk_ok  = ck["is_running"]
    clk_warn= ck["state"] == "STARTING"
    clk_st, clk_col = health_status(clk_ok, clk_warn)
    lt      = ck["last_tick_ago"]
    clk_detail = f"Last tick: {lt}s ago" if lt >= 0 else "Never ticked"
    ...
    try:
        with _db_lock:
            conn = get_conn()
            err_today = conn.execute("SELECT COUNT(*) FROM black_box WHERE level='error' AND ts LIKE ?", (f"{jalali()}%",)).fetchone()[0]
    except Exception:
        err_today = 0
    err_ok = err_today < 10

    health_items = [
        ("⏰ Clock Service",    clk_st, clk_col, clk_detail, "/clock", "ساعت_فعال"),
        ("🗄 Database",         *health_status(db_ok), f"WAL mode | {DB_PATH.split('/')[-1]}", None, None),
        ("💻 RAM",              *health_status(ram_ok, ram_warn), f"{sys['ram']:.1f}% used ({sys['ram_mb']} MB)", None, None),
        ("🛡 Self-Healing",    "🟢 Active", "green", "بررسی هر ۵ دقیقه", None, None),
        ("📦 Black Box",        "🟢 Active", "green", f"{err_today} خطا امروز", "/logs", None),
        ("🔒 Airlock",          "🟡 ACTIVE 🔒" if _airlock_v9 else "🟢 OFF", "yellow" if _airlock_v9 else "green", "", "/settings", None),
        ("📡 Local Tower",     "🟢 Running", "green", f"localhost:{LCT_PORT}", None, None),
        ("❌ Error Rate",       *health_status(err_ok), f"{err_today} خطا امروز", "/logs", None),
    ]

    h = '<div class="page-header"><h2>🩺 Health Center</h2><div class="sub">وضعیت سلامت کل سیستم</div></div>'
    h += '<div class="health-grid">'
    for name, st, col, detail, link, action in health_items:
        h += f'''<div class="health-card">
  <div class="h-title"><span class="dot {col}"></span> {_html.escape(name)}</div>
  <div class="h-status">{_html.escape(st)}</div>
  <div class="h-last">{_html.escape(str(detail))}</div>
  <div class="h-action">
    {f'<a class="btn btn-ghost btn-xs" href="{link}">Details</a>' if link else ""}
    {f'<a class="btn btn-success btn-xs" href="/api/clock?action=healthcheck">Self-Heal</a>' if action == "ساعت_فعال" else ""}
  </div>
</div>'''
    h += '</div>'

    # Recent Black Box summary
    h += '<div class="card" style="margin-top:16px"><div class="card-header">📊 Black Box Summary (24h)</div><div class="card-body">'
    try:
        with _db_lock:
            conn = get_conn()
            rows = conn.execute("""
                SELECT level, COUNT(*) as cnt FROM black_box
                WHERE ts > ? GROUP BY level ORDER BY cnt DESC
            """, (f"{jalali()} 00:00",)).fetchall()
    except Exception:
        rows = []
    if rows:
        h += '<div class="stats-grid" style="grid-template-columns:repeat(auto-fill,minmax(120px,1fr))">'
        for r in rows:
            lv = r["level"]; cnt = r["cnt"]
            col = "red" if lv in ("error","critical") else ("yellow" if lv=="warning" else "blue")
            h += f'<div class="stat-card {col}"><div class="label">{lv.upper()}</div><div class="value">{cnt}</div></div>'
        h += '</div>'
    else:
        h += '<div class="alert ok" style="margin:0">✅ بدون رویداد امروز</div>'
    h += '</div></div>'
    h += '<script>setTimeout(()=>location.reload(),20000)</script>'
    return _lct_page_wrap("health", "Health Center", "System · Clock · DB · Memory", h)

# ══════════════════════════════════════════════════════
#  PAGE: LOGS
# ══════════════════════════════════════════════════════
def _lct_page_logs(level_filter: str = "", search: str = "") -> str:
    with _db_lock:
        conn = get_conn()
        try:
            if level_filter and level_filter != "all":
                rows = conn.execute(
                    "SELECT * FROM black_box WHERE level=? ORDER BY id DESC LIMIT 120", (level_filter,)
                ).fetchall()
            else:
                rows = conn.execute("SELECT * FROM black_box ORDER BY id DESC LIMIT 120").fetchall()
        except Exception:
            rows = []

    if search:
        s = search.lower()
        rows = [r for r in rows if s in str(r.get("event","")).lower() or s in str(r.get("detail","")).lower()]

    levels = ["all","info","warning","error","critical"]
    h = f'<div class="page-header"><h2>📋 Log Center</h2><div class="sub">{len(rows)} رکورد</div></div>'

    # Filter bar
    h += '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;align-items:center">'
    h += '<div class="cmd-filter" style="margin:0">'
    for lv in levels:
        active = "active" if lv == (level_filter or "all") else ""
        h += f'<a class="cmd-tag {active}" href="/logs?level={lv}">{lv.upper()}</a>'
    h += '</div>'
    h += f'<input type="text" class="form-control" style="max-width:220px;flex:1" placeholder="جستجو..." value="{_html.escape(search)}" onchange="location.href=\'/logs?level={level_filter or "all"}&q=\'+encodeURIComponent(this.value)">'
    h += '<a class="btn btn-danger btn-sm" href="/api/clear_logs" onclick="return confirm(\'پاک کنم؟\')">🗑 Clear All</a>'
    h += '</div>'

    h += '<div class="card"><div class="card-body" style="padding:10px">'
    h += '<div class="log-container">'
    if not rows:
        h += '<div style="color:var(--text3);padding:20px;text-align:center">📭 رویدادی برای نمایش نیست</div>'
    else:
        for r in rows:
            lv = str(r["level"]) if "level" in r.keys() else "info"
            ev = _html.escape(str(r.get("event",""))[:60])
            dt = _html.escape(str(r.get("detail",""))[:90])
            ts = str(r.get("ts",""))[:16]
            h += f'<div class="log-line {lv}"><span class="log-ts">[{ts}]</span><span class="log-cat">[{lv.upper()}]</span><span class="log-msg">{ev}: {dt}</span></div>'
    h += '</div></div></div>'
    return _lct_page_wrap("logs", "Log Center", f"{len(rows)} entries", h)

# ══════════════════════════════════════════════════════
#  PAGE: LIVE ACTIVITY
# ══════════════════════════════════════════════════════
def _lct_page_activity() -> str:
    h = '<div class="page-header"><h2>📡 Live Activity</h2><div class="sub">رویدادهای زنده سیستم — auto-refresh هر ۴ ثانیه</div></div>'
    h += '<div class="card"><div class="card-header">📡 Activity Feed <span style="font-size:.72rem;color:var(--text3);font-weight:400">— آخرین ۲۰۰ رویداد</span></div>'
    h += '<div class="card-body"><div class="activity-feed" id="activity-feed" style="height:600px"></div></div></div>'
    return _lct_page_wrap("activity", "Live Activity", "Real-time events", h)

# ══════════════════════════════════════════════════════
#  PAGE: SETTINGS
# ══════════════════════════════════════════════════════
def _lct_page_settings() -> str:
    h = '<div class="page-header"><h2>🔧 Settings Manager</h2><div class="sub">تنظیمات کل سیستم از یک جا</div></div>'
    h += '<div class="two-col">'

    # Clock settings
    cur_sfx  = setting("clock_suffix","")
    cur_font = setting("clock_font","normal")
    h += f'''<div class="card"><div class="card-header">⏰ Clock Settings</div><div class="card-body">
  <div class="form-group">
    <label class="form-label">Font ساعت</label>
    <select id="s_clock_font" class="form-control" onchange="saveSetting('clock_font')">
      {"".join(f'<option value="{fn}" {"selected" if fn==cur_font else ""}>{fn}</option>' for fn in CLOCK_FONTS)}
    </select>
  </div>
  <div class="form-group">
    <label class="form-label">Clock Suffix</label>
    <input type="text" id="s_clock_suffix" class="form-control" value="{_html.escape(cur_sfx)}" placeholder="| 🌙">
    <button class="btn btn-primary btn-sm" style="margin-top:6px" onclick="saveSetting('clock_suffix')">ذخیره</button>
  </div>
  <div style="display:flex;gap:8px;margin-top:10px">
    <a class="btn btn-success btn-sm" href="/api/clock?action=start">▶ Start Clock</a>
    <a class="btn btn-danger btn-sm" href="/api/clock?action=stop">■ Stop</a>
    <a class="btn btn-ghost btn-sm" href="/clock">Details</a>
  </div>
</div></div>'''

    # Store settings
    h += '<div class="card"><div class="card-header">🛍 Store Settings</div><div class="card-body">'
    for key, label, ph in [
        ("card_number","💳 شماره کارت","6037-XXXX-XXXX-XXXX"),
        ("card_holder","👤 صاحب کارت","نام صاحب کارت"),
        ("bank_name","🏦 نام بانک","ملی / ملت / صادرات ..."),
        ("payment_note","📝 توضیح پرداخت","پرداخت در ..."),
    ]:
        val = setting(key,"")
        h += f'''<div class="form-group">
  <label class="form-label">{label}</label>
  <input type="text" id="s_{key}" class="form-control" value="{_html.escape(val)}" placeholder="{_html.escape(ph)}">
  <button class="btn btn-ghost btn-xs" style="margin-top:4px" onclick="saveSetting('{key}')">ذخیره</button>
</div>'''
    h += '</div></div>'
    h += '</div><div class="two-col">'

    # Security
    h += f'''<div class="card"><div class="card-header">🔐 Security Settings</div><div class="card-body">
  <div class="status-list">
    <div class="status-row">
      <span class="status-label">Airlock Mode</span>
      <span class="status-val">{"<span class='badge yellow'>🔒 ACTIVE</span>" if _airlock_v9 else "<span class='badge gray'>OFF</span>"}</span>
    </div>
  </div>
  <div style="display:flex;gap:8px;margin-top:12px">
    <a class="btn btn-danger btn-sm" href="/api/airlock?action=on">🔒 Activate</a>
    <a class="btn btn-success btn-sm" href="/api/airlock?action=off">🔓 Deactivate</a>
  </div>
</div></div>'''

    # Telegram profile quick settings
    h += f'''<div class="card"><div class="card-header">👤 Quick Profile</div><div class="card-body">
  <div class="form-group">
    <label class="form-label">First Name</label>
    <input type="text" id="s_profile_name" class="form-control" placeholder="نام پروفایل">
  </div>
  <button class="btn btn-primary btn-sm" onclick="doAction('/api/profile?field=name&val='+encodeURIComponent(document.getElementById('s_profile_name').value))">تغییر نام</button>
</div></div>'''
    h += '</div>'
    return _lct_page_wrap("settings", "Settings Manager", "Clock · Store · Security · Profile", h)

# ══════════════════════════════════════════════════════
#  API HANDLERS
# ══════════════════════════════════════════════════════
def _lct_api_search(q: str) -> dict:
    q = q.lower().strip()
    results = []
    # Search commands
    for cat_fa, cmds in FULL_MENU.items():
        for cmd in cmds:
            name, desc = cmd[0], cmd[1]
            if q in name.lower() or q in desc.lower() or q in cat_fa.lower():
                results.append({"type":"CMD","name":name,"desc":desc[:50]})
    # Search settings
    try:
        with _db_lock:
            conn = get_conn()
            rows = conn.execute("SELECT key,value FROM settings WHERE key LIKE ?", (f"%{q}%",)).fetchall()
        for r in rows[:5]:
            results.append({"type":"SET","name":r["key"],"desc":f"= {str(r['value'])[:30]}"})
    except Exception:
        pass
    # Search products
    try:
        with _db_lock:
            conn = get_conn()
            prods = conn.execute("SELECT id,name FROM store_products WHERE name LIKE ? LIMIT 5", (f"%{q}%",)).fetchall()
        for p in prods:
            results.append({"type":"PROD","name":p["name"],"desc":f"id={p['id']}"})
    except Exception:
        pass
    # Search CRM
    try:
        with _db_lock:
            conn = get_conn()
            custs = conn.execute("SELECT uid,name FROM crm_customers WHERE name LIKE ? LIMIT 5", (f"%{q}%",)).fetchall()
        for c in custs:
            results.append({"type":"CRM","name":c["name"],"desc":f"uid={c['uid']}"})
    except Exception:
        pass
    # Search orders
    try:
        with _db_lock:
            conn = get_conn()
            orders = conn.execute("SELECT order_uid,product_name FROM store_orders WHERE order_uid LIKE ? OR product_name LIKE ? LIMIT 5", (f"%{q}%",f"%{q}%")).fetchall()
        for o in orders:
            results.append({"type":"ORDER","name":o["order_uid"],"desc":o["product_name"][:40]})
    except Exception:
        pass
    return {"results": results[:15], "total": len(results)}

def _lct_api_status() -> dict:
    ck = _lct_get_clock_real_status()
    return {
        "ok": True,
        "time": iran_now().strftime("%H:%M:%S"),
        "date": jalali(),
        "clock": ck,
        "airlock": _airlock_v9,
        "version": VERSION,
...
#  HTTP SERVER
# ══════════════════════════════════════════════════════
class LCTHandler(BaseHTTPRequestHandler):
    def log_message(self, fmt, *args): pass

    def _send(self, body: str, ct: str = "text/html; charset=utf-8", code: int = 200):
    ...
    def _json(self, data: dict, code: int = 200):
        # Never expose secrets in JSON responses
        safe = {k: v for k, v in data.items() if "key" not in str(k).lower() and "token" not in str(k).lower() and "pass" not in str(k).lower() and "session" not in str(k).lower()}
        self._send(json.dumps(safe, ensure_ascii=False, default=str), "application/json", code)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/api/ai_add":
                ...
                url    = data.get("url","")
                if not key:
                    self._json({"error":"API Key الزامی است"}); return
                # Store API key (base64 encoded — not plaintext in UI)
                encoded_key = base64.b64encode(key.encode()).decode()
                try:
                    with _db_lock:
                        conn = get_conn()
                        conn.execute("""
                            INSERT INTO ai_providers(provider_type,provider_name,api_key,model,base_url,enabled,is_default)
                            VALUES(?,?,?,?,?,1,0)
                        """, (ptype, ptype.title(), encoded_key, model, url))
                        conn.commit()
                    _bb_log_v9("ai_provider_added", f"type={ptype} model={model}", "info")
                    _lct_activity("AI", f"Provider added: {ptype}")
                    self._json({"msg":f"✅ Provider {ptype} اضافه شد"})
                except Exception as e:
    ...
    def do_GET(self):
        global _clock_task, _airlock_v9, _clock_state
        try:
            parsed = urlparse(self.path)
            path   = parsed.path.rstrip("/") or "/"
            qs     = parse_qs(parsed.query)
            def qget(k, d=""): return unquote(qs.get(k,[""])[0] or d)

            # ── Pages ──
            if path in ("/", "/dashboard"):
                _lct_activity("SYSTEM","Dashboard opened")
                self._send(_lct_page_dashboard())
            elif path == "/features":
                self._send(_lct_page_features())
            elif path == "/commands":
                self._send(_lct_page_commands(qget("cat","")))
            elif path == "/clock":
                self._send(_lct_page_clock())
            elif path == "/ai":
                self._send(_lct_page_ai())
            elif path == "/health":
                self._send(_lct_page_health())
            elif path == "/logs":
                self._send(_lct_page_logs(qget("level",""), qget("q","")))
            elif path == "/activity":
                self._send(_lct_page_activity())
            elif path == "/settings":
                self._send(_lct_page_settings())

            # ── API: Ping ──
            elif path == "/api/ping":
                self._json({"ok":True,"ts":_time.time()})

            # ── API: Time ──
            elif path == "/api/time":
                now = iran_now()
                self._json({"date":jalali(now),"time":now.strftime("%H:%M:%S")})

            # ── API: Status ──
            elif path == "/api/status":
                self._json(_lct_api_status())

            # ── API: Search ──
            elif path == "/api/search":
                q = qget("q","")
                self._json(_lct_api_search(q))

            # ── API: Activity ──
            elif path == "/api/activity":
                items = list(_activity_log)[:50]
            ...
            # ── API: Clock ──
                ...
                if action == "start":
                    ...
                    if _lct_client_ref:
                        loop = asyncio.get_event_loop()
                        _clock_task = loop.create_task(_clock_loop_v10(_lct_client_ref))
                        _lct_activity("CLOCK","Clock started from LCT")
                        _bb_log_v9("clock_start_lct","Started from Local Tower","info")
                        self._json({"msg":"✅ Clock started"})
                    else:
                        self._json({"error":"Client not ready"})
                elif action == "stop":
                    _clock_state = "STOPPING"
                    _clock_set_active(False)
                    if _clock_task and not _clock_task.done():
                    ...
                    _clock_task  = None
                    _clock_state = "DISABLED"
                    _lct_activity("CLOCK","Clock stopped from LCT")
                    self._json({"msg":"✅ Clock stopped"})
                elif action == "restart":
                    if _clock_task and not _clock_task.done():
                        _clock_task.cancel()
                    _clock_state = "STARTING"
                    if _lct_client_ref:
                        loop = asyncio.get_event_loop()
                        _clock_task = loop.create_task(_clock_loop_v10(_lct_client_ref))
                        _lct_activity("CLOCK","Clock restarted from LCT")
                        self._json({"msg":"✅ Clock restarted"})
                    else:
                        self._json({"error":"Client not ready"})
                elif action == "healthcheck":
                    st = _lct_get_clock_real_status()
                    if st["state"] == "FAILED" and _lct_client_ref:
                        if _clock_task and not _clock_task.done():
                            _clock_task.cancel()
                        loop = asyncio.get_event_loop()
                        _clock_task = loop.create_task(_clock_loop_v10(_lct_client_ref))
                        _bb_log_v9("clock_recovered","LCT Health Check recovery","info")
                        _lct_activity("CLOCK","Clock recovered via Health Check")
                        self._json({"msg":"✅ Clock recovered"})
                    else:
                        self._json({"msg":f"State: {st['state']}", "state":st["state"]})
                else:
                    self._json({"error":"Unknown clock action"})
            ...
            # ── API: Feature ──
                ...
                action = qget("action","")
                if not fid:
                    self._json({"error":"Missing id"}); return
                db_key = f"feat_disabled_{fid}"
                if action == "disable":
                    set_setting(db_key, "1")
                    _bb_log_v9("feat_disabled",f"feat={fid}","info")
                    _lct_activity("SYSTEM",f"Feature disabled: {fid}")
                    self._json({"msg":f"✅ Feature '{fid}' disabled"})
                elif action == "enable":
                    try:
                        with _db_lock:
                            conn = get_conn()
                        ...
                        pass
                    _bb_log_v9("feat_enabled",f"feat={fid}","info")
                    _lct_activity("SYSTEM",f"Feature enabled: {fid}")
                    self._json({"msg":f"✅ Feature '{fid}' enabled"})
                else:
            ...
            # ── API: Setting ──
                ...
                val = qget("val","")
                SAFE_KEYS = {
                    "clock_font","clock_suffix","card_number","card_holder","bank_name","payment_note",
                    "clock_active","ai_max_req","ai_ctx_limit","ai_timeout","ai_cache","ai_enabled",
                    "ai_feat_provider_smart_neg","ai_feat_provider_ctx_switch","ai_feat_provider_honeytrap",
                    "ai_feat_provider_product_know","ai_feat_provider_identity","ai_feat_provider_link_intel",
                    "ai_feat_model_smart_neg","ai_feat_model_ctx_switch","ai_feat_model_honeytrap",
                    "ai_feat_model_product_know","ai_feat_model_identity","ai_feat_model_link_intel",
                }
                if not key or key not in SAFE_KEYS:
                    self._json({"error":f"Key not allowed: {key}"}); return
                # Never allow secrets in value
                if any(s in val.lower() for s in ["api_key","token","password","session"]):
                    self._json({"error":"Value not allowed"}); return
                set_setting(key, val)
                _lct_activity("SYSTEM",f"Setting: {key}={val[:20]}")
                self._json({"msg":f"✅ {key} saved"})

            # ── API: Airlock ──
            elif path == "/api/airlock":
                action = qget("action","")
                if action == "on":
                    _airlock_v9 = True
                    set_setting("airlock_active","1")
                    _bb_log_v9("airlock_on","LCT","warning")
                    _lct_activity("SECURITY","Airlock ACTIVATED")
                    self._json({"msg":"🔒 Airlock activated"})
                elif action == "off":
                    _airlock_v9 = False
                    set_setting("airlock_active","0")
                    _bb_log_v9("airlock_off","LCT","info")
                    _lct_activity("SECURITY","Airlock deactivated")
                    self._json({"msg":"🔓 Airlock off"})
                else:
                    self._json({"error":"Unknown"})

            # ── API: AI toggle ──
            elif path == "/api/ai_toggle":
                cur = setting("ai_enabled","1")
                new_val = "0" if cur=="1" else "1"
                set_setting("ai_enabled",new_val)
                _lct_activity("AI",f"AI {'enabled' if new_val=='1' else 'disabled'}")
                self._json({"msg":f"AI {'enabled' if new_val=='1' else 'disabled'}"})

            # ── API: AI provider toggle ──
            elif path == "/api/ai_toggle_provider":
                pid = qget("id","")
                try:
                    with _db_lock:
                        conn = get_conn()
                        row = conn.execute("SELECT enabled FROM ai_providers WHERE id=?", (pid,)).fetchone()
                    if row:
                        new_en = 0 if row["enabled"] else 1
                        with _db_lock:
                            conn = get_conn()
                            conn.execute("UPDATE ai_providers SET enabled=? WHERE id=?", (new_en, pid))
                            conn.commit()
                        self._json({"msg":f"Provider {'enabled' if new_en else 'disabled'}"})
                    else:
                        self._json({"error":"Provider not found"})
                except Exception as e:
                    self._json({"error":str(e)})

            # ── API: AI default ──
            elif path == "/api/ai_default":
                pid = qget("id","")
                try:
                    with _db_lock:
                        conn = get_conn()
                        conn.execute("UPDATE ai_providers SET is_default=0")
                        conn.execute("UPDATE ai_providers SET is_default=1 WHERE id=?", (pid,))
                        conn.commit()
                    self._json({"msg":"✅ Default provider set"})
                except Exception as e:
                    self._json({"error":str(e)})

            # ── API: AI delete ──
            elif path == "/api/ai_delete":
                pid = qget("id","")
                try:
                    with _db_lock:
                        conn = get_conn()
                        ...
                        conn.commit()
                    _lct_activity("AI",f"Provider deleted id={pid}")
                    self._json({"msg":"✅ Provider deleted"})
                except Exception as e:
                    self._json({"error":str(e)})
            ...
            # ── API: AI test ──
                ...
                try:
                    with _db_lock:
                        conn = get_conn()
                        row = conn.execute("SELECT * FROM ai_providers WHERE id=?", (pid,)).fetchone()
                    if not row:
                        self._json({"ok":False,"error":"Provider not found"}); return
                    # Decode API key
                    ptype    = row["provider_type"]
                    encoded  = row.get("api_key","")
                    model    = row.get("model","")
                    try:
                    ...
                    if ptype == "gemini":
                        ...
                        try:
                            req = _ureq.urlopen(test_url, timeout=8)
                            data_resp = json.loads(req.read().decode())
                            test_ok = "models" in data_resp
                        except Exception as e:
                            test_err = str(e)[:80]
                    elif ptype in ("openai","anthropic","custom"):
                        base_url = row.get("base_url","") or ("https://api.openai.com/v1" if ptype=="openai" else "")
                        if base_url:
                            try:
                                req = _ureq.Request(f"{base_url}/models", headers={"Authorization":f"Bearer {api_key}"})
                                resp = _ureq.urlopen(req, timeout=8)
                                test_ok = resp.status == 200
                            except Exception as e:
                                test_err = str(e)[:80]
                        else:
                            test_err = "Base URL not set"
                    else:
                        test_ok  = True   # local model — assume ok
                    if test_ok:
                        _lct_activity("AI",f"Provider test OK: {ptype}")
                        self._json({"ok":True,"provider":ptype,"model":model})
                    else:
                        self._json({"ok":False,"error":test_err or "Connection failed"})
                except Exception as e:
            ...
            # ── API: Clear logs ──
                ...
                with _db_lock:
                    ...
                    conn.execute("DELETE FROM black_box")
                    conn.commit()
                _lct_activity("SYSTEM","Logs cleared from LCT")
                self._json({"msg":"✅ Logs cleared"})

            # ── API: Profile ──
            elif path == "/api/profile":
                field = qget("field","")
                val   = qget("val","")
                if field == "name" and val and _lct_client_ref:
                    try:
                        loop = asyncio.get_event_loop()
                        loop.create_task(_lct_client_ref(UpdateProfileRequest(first_name=val[:64])))
                        _lct_activity("PROFILE",f"Name changed: {val[:20]}")
                        self._json({"msg":"✅ نام پروفایل تغییر کرد"})
                    except Exception as e:
                        self._json({"error":str(e)})
                else:
                    self._json({"error":"Client not ready or invalid field"})
...
def _start_lct_server():
    """اجرای Local Control Tower در Thread جداگانه"""
    # اضافه کردن جدول ai_providers اگر وجود نداشت
    try:
        with _db_lock:
            conn = get_conn()
            conn.execute("""
    ...
    try:
        server = HTTPServer((LCT_HOST, LCT_PORT), LCTHandler)
        logger.info(f"🖥️  Local Control Tower: http://{LCT_HOST}:{LCT_PORT}")
        print(f"\n🖥️  Local Control Tower: \033[1;36mhttp://{LCT_HOST}:{LCT_PORT}\033[0m")
        server.serve_forever()
    except OSError as e:
        if "already in use" in str(e).lower():
            try:
                alt_port = LCT_PORT + 1
                server2 = HTTPServer((LCT_HOST, alt_port), LCTHandler)
                logger.info(f"🖥️  Local Control Tower (alt): http://{LCT_HOST}:{alt_port}")
                print(f"\n🖥️  Local Control Tower: http://{LCT_HOST}:{alt_port}")
                server2.serve_forever()
            except Exception as e2:
                logger.error(f"🖥️ LCT failed: {e2}")
        else:
            logger.error(f"🖥️ LCT error: {e}")
    except Exception as e:
        logger.error(f"🖥️ LCT unexpected: {e}")
...
if __name__ == "__main__":
    # ── مرحله ۰: بررسی و نصب Dependencies ──────────────
    if not _check_and_install_deps():
        print("\n❌ نصب کتابخانه‌ها ناموفق بود.")
        print("   دستی اجرا کنید: pip install telethon jdatetime")
        sys.exit(1)

    # ── مرحله ۱: پردازش آرگومان‌های خط فرمان ────────────
    ...
    try:
        ...
        print("\n👋 خروج از ONYX SELF V.11")
    except Exception as e:
        err_str = str(e)
        # هرگز اطلاعات حساس را در خطا نشان نده
        cfg_check = _load_config()
        for sensitive in [
            cfg_check.get("api_hash",""), cfg_check.get("gemini_key",""), cfg_check.get("phone","")
        ]:
            if sensitive and sensitive in err_str:
                err_str = err_str.replace(sensitive, "***")
        print(f"\n❌ خطای بحرانی: {err_str}")
        print()
        # ── پیشنهاد راه‌حل ──
        low = err_str.lower()
        if "api_id" in low or "api_hash" in low or "unauthorized" in low:
            print("  ⚠️  احتمالاً API ID یا HASH اشتباه است.")
            print("  راه‌حل: python main.py --setup")
        elif "session" in low or "database" in low:
            print("  ⚠️  Session مشکل دارد.")
            print("  راه‌حل: python main.py --setup  ← گزینه Reset Session")
        elif "flood" in low or "too many" in low:
            print("  ⚠️  محدودیت Telegram. چند دقیقه صبر کنید.")
        elif "import" in low or "module" in low:
            print("  ⚠️  یک کتابخانه کم است.")
            print("  راه‌حل: pip install telethon jdatetime")
        traceback.print_exc()
        sys.exit(1)
